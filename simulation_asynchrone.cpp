...
// ### Pour une raison qui m'échappe TOTALEMENT, en appelant ce fichier simulation_async_affiche_mpi,
// ### je n'arrive pas à le compiler.
// ### Je n'arrive pas à identifier la raison de la Segmentation Fault
...
void simulation(bool affiche)
{
    ...
    MPI_Request request, quit_request;
    ...
    //épidémie::Grille grille_affichage{contexte.taux_population}; // enlever ?
    std::vector<épidémie::Grille::StatistiqueParCase> statistiques;
    statistiques.reserve(contexte.taux_population);
    statistiques = grille.getStatistiques();

    //std::vector<épidémie::Grille::StatistiqueParCase> recv_buffer = grille.getStatistiques(); //enlever ?
    ...
    if (rank > 0)
    {
        MPI_Irecv(&exit_display, 1, MPI_C_BOOL, 0, 1, globComm, &quit_request);
    }
    ...
    while (!quitting)
    {
        if (rank==1) {
            ...
            // Envoi
            statistiques = grille.getStatistiques();
            MPI_Isend(statistiques.data(), 3*largeur_grille*hauteur_grille, MPI_INT, 0, 0, globComm, &request);

            MPI_Test(&quit_request, &quit_flag, MPI_STATUS_IGNORE);
            if (quit_flag)
            {
                quitting = true;
            }
        ...
        if (rank==0) {
            ...
            for ( const auto& e : events)
            {
                if (e->kind_of_event() == sdl2::event::quit) {
                    quitting = true;
                    exit_display = true;
                    MPI_Isend(&exit_display, 1, MPI_C_BOOL, 1, 1, globComm, &quit_request);
                }
            }

            // Reception
            MPI_Iprobe(1, 0, globComm, &flag, MPI_STATUS_IGNORE);
            if (flag) {
                MPI_Recv(statistiques.data(), 3*largeur_grille*hauteur_grille, MPI_INT, 1, 0, globComm, MPI_STATUS_IGNORE);
            }

            grille.setStatistiques(statistiques);
